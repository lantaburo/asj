...
export async function scanAttendanceWithPhoto(
  payload: { sessionId: string; selfieUrl: string },
  currentUserId: string
) {
  const session = await findSessionById(payload.sessionId);
  if (!session) {
    throw new AppError("Session tidak ditemukan.", { statusCode: 404, code: "SESSION_NOT_FOUND" });
  }
  ...
  if (!enrollment) {
    throw new AppError("Peserta belum terdaftar pada batch session ini.", { statusCode: 409, code: "ENROLLMENT_REQUIRED" });
  }
  ...
  try {
    ...
    if (
      ...
      throw new AppError("Kehadiran untuk sesi ini sudah pernah direkam.", { statusCode: 409, code: "DUPLICATE_ATTENDANCE" });
    }
    throw error;
  }
}
